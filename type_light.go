...
type Light struct {
	...
	StartTime, StopTime      time.Duration // The duration after 00:00:00 local time that the light should start its cycle or end its cycle
}
...
func (l *Light) Cycle(mark time.Time) (color uint32) {
	var t time.Duration
	if l.On {
		t = l.OffDuration
		l.On = false
